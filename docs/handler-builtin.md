...
command. See [handlers.md](handlers.md) for the handler model and how
to add your own.
...
| Command | Default Metric Name | Type | Meaning |
|---------|--------|------|---------|
| `builtin.users.count` | `sms_builtin_users_count` | gauge | distinct logged-in users (`who`) |
| `builtin.users.sessions` | `sms_builtin_users_sessions` | gauge | active login sessions (`who`) |
| `builtin.users.by_user` | `sms_builtin_user_id` | gauge | numeric user ID of each logged-in user, labeled: `name{user="alice"} 1000.0` (resolved via `pwd`, `0.0` if unresolvable); renders HELP/TYPE only (no samples) when nobody is logged in |
| `builtin.os.processes` | `sms_builtin_os_processes` | gauge | process count (`ps aux`, header skipped) |
| `builtin.os.packages_upgradable` | `sms_builtin_os_packages_upgradable` | gauge | upgradable packages — `apt-get -s upgrade` (`Inst ` lines) on Debian, `dnf -q check-update`/`yum` on the Red Hat family |
| `builtin.os.reboot_required` | `sms_builtin_os_reboot_required` | gauge | `1.0` if a reboot is due, else `0.0` — `/run/reboot-required` on Debian, `needs-restarting -r` on the Red Hat family |

### Hardware

| Command | Default Metric Name | Type | Meaning |
|---------|--------|------|---------|
| `builtin.cpu.count` | `sms_builtin_cpu_count` | gauge | logical CPUs (`os.cpu_count()`, no subprocess; `0.0` if undeterminable) |
| `builtin.cpu.load1` / `.load5` / `.load15` | `sms_builtin_cpu_load1/5/15` | gauge | load averages (`/proc/loadavg`) |
| `builtin.memory.total_bytes` / `.available_bytes` / `.used_bytes` | `sms_builtin_memory_*_bytes` | gauge | physical memory (`/proc/meminfo`, kB × 1024; used = total − available) |
| `builtin.disk.total_bytes` / `.used_bytes` / `.available_bytes` | `sms_builtin_disk_*_bytes` | gauge | summed over real filesystems (`df -P`, only `/dev/` entries) |
| `builtin.network.received_bytes` / `.transmitted_bytes` | `sms_builtin_network_*_bytes` | gauge | summed over non-loopback interfaces (`/proc/net/dev`) |
